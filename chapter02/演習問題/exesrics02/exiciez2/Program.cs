namespace exiciez2
{
    ...
    {
        static double feetmeta(int inti)
        {
            return inti * 0.0254;
        }
        ...
        static void printfeetmetalist(int start, int stop) //inti→m
        {
            for (int inti = start; inti <= stop; inti++)
            {
                double meta = FeetConverter2.tometer2(inti);
                Console.WriteLine($"{inti}ft={meta:0.0000}m");
            }

        }
        static void printmetftlist(int start, int stop) //m→imti
        {
            for (int met = start; met <= stop; met++)
            {
                double fet = FeetConverter2.Frommetar2(met);
